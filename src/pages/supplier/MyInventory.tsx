...
// ─── Design tokens (matches existing portal theme) ────────────────────────────
const NAVY   = '#0D1B2A';
const TEAL   = '#0097A7';
const MID    = '#354A5F';
const MUTED  = '#64748B';
...
// ─── Current supplier context (Berlina Packaging = sup-007 in demo) ───────────
// In Phase 2 this comes from JWT claims; for now we show all inventory
// filtered to the demo supplier, with an "All Suppliers" fallback for buyer view.
...
function DaysBar({ days }: { days: number }) {
  const status: StockStatus =
    days < 7 ? StockStatus.CRITICAL :
    days < 14 ? StockStatus.LOW :
    days <= 30 ? StockStatus.NORMAL : StockStatus.EXCESS;
  const cfg = STATUS_CFG[status];
  const pct = Math.min((days / 45) * 100, 100);
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <div style={{ flex: 1, background: '#F1F5F9', borderRadius: 4, height: 6, minWidth: 60 }}>
        <div style={{ width: `${pct}%`, background: cfg.bar, height: 6, borderRadius: 4, transition: 'width 0.3s' }} />
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: MUTED }}>
            Live stock visibility across all materials · Days-of-supply tracking · Paragon minimum thresholds
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            onClick={() => showToast('Inventory sync requested — updating from supplier API feeds...')}
            style={{
          ...
          <button
            onClick={() => showToast('Export preparing... EDI 846 format download starting')}
            style={{
        ...
        {([
          ...
          <div
            ...
            style={{
              ...
              border: `1px solid ${filterStatus === filter ? bar : BORDER}`,
          ...
          >
            ...
            <div style={{ fontSize: 11, color: MUTED, marginTop: 4 }}>
              {((count / allInventory.length) * 100).toFixed(0)}% of materials
            </div>
          </div>
        ))}
      </div>
      ...
      {counts.critical > 0 && (
        <div style={{
          ...
          <span>
            <strong>{counts.critical} material{counts.critical > 1 ? 's' : ''}</strong> at critical stock level (&lt;7 days of supply).
            Paragon procurement team has been automatically notified.
          </span>
        </div>
      )}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: NAVY, color: 'white' }}>
              {[
                'Material Code', 'Description', 'Supplier',
                'On Hand', 'Available', 'In Transit',
                'UoM', 'Days Supply', 'Status', 'Source', 'Last Updated',
              ].map(h => (
                <th key={h} style={{
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={11} style={{ padding: '2rem', textAlign: 'center', color: MUTED, fontSize: 13 }}>
              ...
              return (
                ...
                >
                  <td style={{ padding: '10px 12px', fontFamily: 'var(--font-mono)', fontSize: 11, color: TEAL, fontWeight: 600 }}>
                    {row.materialCode}
                  </td>
                  ...
                  <td style={{ padding: '10px 12px', color: MID, fontSize: 12 }}>
                    {row.supplierName.replace('PT ', '').split(' ').slice(0, 2).join(' ')}
                  </td>
                  <td style={{ padding: '10px 12px', fontWeight: 600, color: NAVY, textAlign: 'right' }}>
                    {fmt(row.qtyOnHand)}
                  </td>
                  <td style={{ padding: '10px 12px', color: NAVY, textAlign: 'right' }}>
                    {fmt(row.qtyAvailable)}
                  </td>
                  <td style={{ padding: '10px 12px', color: row.qtyInTransit > 0 ? '#0A6ED1' : MUTED, textAlign: 'right', fontStyle: row.qtyInTransit === 0 ? 'italic' : 'normal' }}>
                  ...
                  <td style={{ padding: '10px 12px', minWidth: 120 }}>
                    <DaysBar days={row.daysOfSupply} />
                  </td>
                  <td style={{ padding: '10px 12px' }}>
                    <Pill label={cfg.label} bg={cfg.bg} color={cfg.color} />
                  </td>
                  <td style={{ padding: '10px 12px' }}>
                    <Pill label={row.dataSource} bg={srcBg} color={srcColor} />
                  </td>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div style={{
          ...
          <span>📡</span>
          <span>
            <strong>Data sources:</strong> API Push (real-time), EDI 846 (daily), Manual (supplier-updated).
            Phase 2 will add SAP MM stock pull and VMI signal integration.
          </span>
        </div>
        <div style={{
          ...
          <span>⏱</span>
          <span>
            <strong>Thresholds:</strong> Critical &lt;7 days · Low 7–14 days · Normal 14–30 days · Excess &gt;30 days.
            Paragon minimum stock requirements are enforced at category level.
          </span>
        </div>
      </div>
    </div>
