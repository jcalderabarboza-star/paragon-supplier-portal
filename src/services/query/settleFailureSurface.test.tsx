...
// below is believed.
//
// ⚠️ §44 · IF YOU ARE HERE TO WRITE AN INVOICE-PATH FAILURE TEST, READ THIS
// FIRST — THE COVERAGE ACROSS THE TWO SETTLE PATHS IS ASYMMETRIC, BY A FACTOR
// OF SIX. There are exactly two `sapBoundary` verbs, so exactly two paths where
// a settle is reachable at all: `t_gr_post` (`useGoodsReceiptSettle`) and
// `t_invoice_release_payment` (`useInvoiceSettlePayment`). Of the tests below,
// the invoice path carries ONE — the shared-vocabulary case. All three fault
// classes (TRANSPORT / UNGOVERNED / REFUSED) and BOTH locales are asserted only
// through `GrProbe`. The classifier is path-independent, so this is not
// fictitious coverage; but **if the invoice path ever diverges — a different
// toast, a different remedy sentence, an ID string that does not follow — six of
// the seven assertions here would not notice.** Add the invoice twin of whichever
// case you are touching rather than only the case itself.
//
// ⚠️ AND ONE THING YOU MAY HAVE BEEN TOLD THAT IS NOT TRUE OF THIS TREE:
// **there is no `Math.random` call-order coupling between a settle failure and
// the correlation id, because CORRELATION IDS ARE NOT RANDOM.** They come from a
// monotonic counter (`MockCommandService.ts`, `nextCorrelationId`, commented
// "deterministic for tests"). You CAN make a settle fail while keeping the id
// stable — `failingService()` below does exactly that, and `CORRELATION` is a
// fixed string. Stubbing `Math.random` is neither necessary nor sufficient here.
// ─────────────────────────────────────────────────────────────────────────────
