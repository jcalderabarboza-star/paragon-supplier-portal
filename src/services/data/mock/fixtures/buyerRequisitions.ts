...
//
// ⚠️ §69 — `approvalLevel` IS AUTHORED HERE AND NOWHERE ELSE. The values
// correlate with `estimatedValue` and NOTHING derives them; the correlation is
// this file's authorship, not a rule. Do not reverse-engineer thresholds from
// these six rows — they constrain only ≤43M, [67M,105M] and ≥210M, and the two
// gaps are unassigned by any evidence in the tree. `''` means NOT ASSIGNED, and
// it is the one representation (`t_pr_create` writes the same).
//
