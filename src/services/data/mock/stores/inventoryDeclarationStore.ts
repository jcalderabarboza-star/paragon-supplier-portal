...
// — SOH is keyed material + declaredAt, one true SOH per as-of). "Current SOH"
// is derived at read via `latestFor` (latest declaredAt per supplier ×
// material). No `update` path exists — snapshots don't change; a new count is
...
import { INVENTORY_DECLARATIONS } from '../../../sdc';
...
export const inventoryDeclarationStore = {
  ...
  /**
   * The CURRENT SOH declaration for a supplier × material — the latest by
   * `declaredAt` (a state snapshot, not a running total). Undefined when the
   * pair has never declared.
   */
  latestFor(supplierId: string, materialCode: string): InventoryDeclaration | undefined {
    ...
    for (const d of rows) {
      ...
      if (!latest || Date.parse(d.declaredAt) > Date.parse(latest.declaredAt)) latest = d;
    }
    return latest;
  },
