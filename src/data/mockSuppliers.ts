import { Supplier } from '../types/supplier.types';

// Extended supplier type for the Paragon beauty & personal care supply chain
export interface ParagonSupplier extends Supplier {
  sapBpNumber: string;
  city: string;
  tier: 1 | 2 | 3; // 1=WhatsApp, 2=Web Portal, 3=API/EDI
  contactEmail: string;
  contactPhone: string;
  halalCertified: boolean;
  bpomRegistered: boolean;
  certExpiryDate: string;
  preferredChannel: 'WhatsApp' | 'Email' | 'Web' | 'API';
  otif: number;
  leadTimeAdherence: number;
  invoiceAccuracy: number;
  scorecardGrade: 'A' | 'B' | 'C' | 'D';
  lastActivityDate: string;
}

// status mapping: 'active' = Active, 'pending' = Onboarding, 'inactive' = Suspended
export const mockSuppliers: ParagonSupplier[] = [
  {
    id: 'sup-001',
    sapBpNumber: 'BP-10001234',
    name: 'PT Ecogreen Oleochemicals',
    category: 'Raw Material',
    country: 'ID',
    city: 'Batam',
    tier: 3,
    status: 'active',
    contactName: 'Budi Santoso',
    email: 'budi.santoso@ecogreen.co.id',
    contactEmail: 'budi.santoso@ecogreen.co.id',
    phone: '+6281298765001',
    contactPhone: '+6281298765001',
    rating: 4.5,
    onboardedDate: '2021-03-10',
    lastActivityDate: '2025-04-04',
    halalCertified: true,
    bpomRegistered: true,
    certExpiryDate: '2026-03-09',
    preferredChannel: 'API',
    otif: 95,
    leadTimeAdherence: 93,
    invoiceAccuracy: 99,
    scorecardGrade: 'A',
  },
  {
    id: 'sup-002',
    sapBpNumber: 'BP-10001235',
    name: 'PT Musim Mas Specialty Fats',
    category: 'Raw Material',
    country: 'ID',
    city: 'Medan',
    tier: 2,
    status: 'active',
    contactName: 'Ratna Dewi',
    email: 'ratna.dewi@musimmas.co.id',
    contactEmail: 'ratna.dewi@musimmas.co.id',
    phone: '+6281234567002',
    contactPhone: '+6281234567002',
    rating: 4.2,
    onboardedDate: '2020-07-15',
    lastActivityDate: '2025-04-01',
    halalCertified: true,
    bpomRegistered: true,
    certExpiryDate: '2025-06-10', // near expiry — triggers alert
    preferredChannel: 'Web',
    otif: 91,
    leadTimeAdherence: 88,
    invoiceAccuracy: 96,
    scorecardGrade: 'A',
  },
  {
    id: 'sup-003',
    sapBpNumber: 'BP-10001236',
    name: 'Givaudan Indonesia Fragrances',
    category: 'Fragrance',
    country: 'ID',
    city: 'Jakarta',
    tier: 3,
    status: 'active',
    contactName: 'Siti Marlina',
    email: 's.marlina@givaudan.com',
    contactEmail: 's.marlina@givaudan.com',
    phone: '+6281187654003',
    contactPhone: '+6281187654003',
    rating: 4.7,
    onboardedDate: '2019-11-01',
    lastActivityDate: '2025-04-05',
    halalCertified: true,
    bpomRegistered: true,
    certExpiryDate: '2026-11-01',
    preferredChannel: 'API',
    otif: 97,
    leadTimeAdherence: 96,
    invoiceAccuracy: 100,
    scorecardGrade: 'A',
  },
  {
    id: 'sup-004',
    sapBpNumber: 'BP-10001237',
    name: 'Firmenich Malaysia Sdn. Bhd.',
    category: 'Fragrance',
    country: 'MY',
    city: 'Petaling Jaya',
    tier: 2,
    status: 'active',
    contactName: 'Ahmad Fauzi',
    email: 'a.fauzi@firmenich.com.my',
    contactEmail: 'a.fauzi@firmenich.com.my',
    phone: '+60112345004',
    contactPhone: '+60112345004',
    rating: 4.3,
    onboardedDate: '2021-06-20',
    lastActivityDate: '2025-03-28',
    halalCertified: true,
    bpomRegistered: false,
    certExpiryDate: '2026-06-19',
    preferredChannel: 'Web',
    otif: 89,
    leadTimeAdherence: 85,
    invoiceAccuracy: 97,
    scorecardGrade: 'B',
  },
  {
    id: 'sup-005',
    sapBpNumber: 'BP-10001238',
    name: 'BASF Personal Care Emulsifiers GmbH',
    category: 'Active Ingredient',
    country: 'DE',
    city: 'Ludwigshafen',
    tier: 3,
    status: 'active',
    contactName: 'Klaus Becker',
    email: 'k.becker@basf.com',
    contactEmail: 'k.becker@basf.com',
    phone: '+4962158005',
    contactPhone: '+4962158005',
    rating: 4.6,
    onboardedDate: '2018-02-14',
    lastActivityDate: '2025-04-03',
    halalCertified: false,
    bpomRegistered: true,
    certExpiryDate: '2026-02-13',
    preferredChannel: 'API',
    otif: 93,
    leadTimeAdherence: 92,
    invoiceAccuracy: 98,
    scorecardGrade: 'A',
  },
  {
    id: 'sup-006',
    sapBpNumber: 'BP-10001239',
    name: 'Evonik Specialty Chemicals France',
    category: 'Active Ingredient',
    country: 'FR',
    city: 'Paris',
    tier: 2,
    status: 'active',
    contactName: 'Claire Dupont',
    email: 'c.dupont@evonik.com',
    contactEmail: 'c.dupont@evonik.com',
    phone: '+33140556006',
    contactPhone: '+33140556006',
    rating: 4.1,
    onboardedDate: '2020-04-05',
    lastActivityDate: '2025-04-02',
    halalCertified: false,
    bpomRegistered: true,
    certExpiryDate: '2025-05-04', // near expiry — triggers alert
    preferredChannel: 'Email',
    otif: 87,
    leadTimeAdherence: 84,
    invoiceAccuracy: 95,
    scorecardGrade: 'B',
  },
  {
    id: 'sup-007',
    sapBpNumber: 'BP-10001240',
    name: 'PT Berlina Packaging Indonesia',
    category: 'Packaging',
    country: 'ID',
    city: 'Tangerang',
    tier: 1,
    status: 'active',
    contactName: 'Hendra Wijaya',
    email: 'hendra.wijaya@berlina.co.id',
    contactEmail: 'hendra.wijaya@berlina.co.id',
    phone: '+6281398765007',
    contactPhone: '+6281398765007',
    rating: 3.8,
    onboardedDate: '2022-09-01',
    lastActivityDate: '2025-04-05',
    halalCertified: false,
    bpomRegistered: false,
    certExpiryDate: '2025-09-01',
    preferredChannel: 'WhatsApp',
    otif: 82,
    leadTimeAdherence: 80,
    invoiceAccuracy: 91,
    scorecardGrade: 'C',
  },
  {
    id: 'sup-008',
    sapBpNumber: 'BP-10001241',
    name: 'PT Indo Karton Packaging',
    category: 'Packaging',
    country: 'ID',
    city: 'Surabaya',
    tier: 1,
    status: 'active',
    contactName: 'Andi Prasetyo',
    email: 'andi.prasetyo@indokarton.co.id',
    contactEmail: 'andi.prasetyo@indokarton.co.id',
    phone: '+6285712345008',
    contactPhone: '+6285712345008',
    rating: 3.5,
    onboardedDate: '2023-01-10',
    lastActivityDate: '2025-03-30',
    halalCertified: false,
    bpomRegistered: false,
    certExpiryDate: '2026-01-09',
    preferredChannel: 'WhatsApp',
    otif: 79,
    leadTimeAdherence: 81,
    invoiceAccuracy: 88,
    scorecardGrade: 'C',
  },
  {
    id: 'sup-009',
    sapBpNumber: 'BP-10001242',
    name: 'Zhejiang NHU Vitamins Co.',
    category: 'Active Ingredient',
    country: 'CN',
    city: 'Hangzhou',
    tier: 2,
    status: 'active',
    contactName: 'Li Wei',
    email: 'li.wei@zj-nhu.com',
    contactEmail: 'li.wei@zj-nhu.com',
    phone: '+8657188009009',
    contactPhone: '+8657188009009',
    rating: 4.0,
    onboardedDate: '2021-08-25',
    lastActivityDate: '2025-04-01',
    halalCertified: false,
    bpomRegistered: true,
    certExpiryDate: '2026-08-24',
    preferredChannel: 'Web',
    otif: 85,
    leadTimeAdherence: 83,
    invoiceAccuracy: 93,
    scorecardGrade: 'B',
  },
  {
    id: 'sup-010',
    sapBpNumber: 'BP-10001243',
    name: 'PT Halal Emulsifier Nusantara',
    category: 'Raw Material',
    country: 'ID',
    city: 'Bogor',
    tier: 1,
    status: 'pending',
    contactName: 'Fajar Nugroho',
    email: 'fajar.nugroho@henusan.co.id',
    contactEmail: 'fajar.nugroho@henusan.co.id',
    phone: '+6281567890010',
    contactPhone: '+6281567890010',
    rating: 2.5,
    onboardedDate: '2025-02-01',
    lastActivityDate: '2025-04-05',
    halalCertified: true,
    bpomRegistered: false,
    certExpiryDate: '2025-08-31',
    preferredChannel: 'WhatsApp',
    otif: 75,
    leadTimeAdherence: 80,
    invoiceAccuracy: 85,
    scorecardGrade: 'D',
  },
  {
    id: 'sup-011',
    sapBpNumber: 'BP-10001244',
    name: 'Anhui Salicylics & Niacinamide Ltd.',
    category: 'Active Ingredient',
    country: 'CN',
    city: 'Hefei',
    tier: 2,
    status: 'pending',
    contactName: 'Zhang Mei',
    email: 'zhang.mei@anhui-sal.com',
    contactEmail: 'zhang.mei@anhui-sal.com',
    phone: '+8655191011011',
    contactPhone: '+8655191011011',
    rating: 3.5,
    onboardedDate: '2025-01-15',
    lastActivityDate: '2025-03-20',
    halalCertified: false,
    bpomRegistered: false,
    certExpiryDate: '2026-01-14',
    preferredChannel: 'Email',
    otif: 80,
    leadTimeAdherence: 82,
    invoiceAccuracy: 90,
    scorecardGrade: 'C',
  },
  {
    id: 'sup-012',
    sapBpNumber: 'BP-10001245',
    name: 'Caelo PET Bottle Manufacturer GmbH',
    category: 'Packaging',
    country: 'DE',
    city: 'Hilden',
    tier: 3,
    status: 'inactive',
    contactName: 'Petra Schulz',
    email: 'p.schulz@caelo.de',
    contactEmail: 'p.schulz@caelo.de',
    phone: '+4921038912012',
    contactPhone: '+4921038912012',
    rating: 1.5,
    onboardedDate: '2022-05-01',
    lastActivityDate: '2024-11-10',
    halalCertified: false,
    bpomRegistered: false,
    certExpiryDate: '2024-04-30',
    preferredChannel: 'API',
    otif: 75,
    leadTimeAdherence: 78,
    invoiceAccuracy: 86,
    scorecardGrade: 'D',
  },
];
